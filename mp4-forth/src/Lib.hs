...
liftCompOp op  (x:y:xs) = if (y `op` x) then Just $ (-1) : xs else Just$ 0 : xs 
...
printStack (istack, dict, out) =(istack, dict, out' : out) 
    ---where out' = unwords (map show (reverse istack))
       where out'=(unwords . map show . reverse) istack
------why this does not work?    [(unwords . map show . reverse) istack]
------show out':out
--- Evaluator
--- ---------
...
--- Compiler
--- --------
---"compile" a user definition in Forth to a continuation in Haskell
--- ### Definite Loops
...
transForLoop kloop (i:is, d, o) =
    if i < 0 then
        ...
        (aux kloop i) (is, d, o)  
 ---If i is non-negative then it compose kloop with itself i times via aux function and apply it on the state.
        where aux k 0 = k
              aux k n = k . (aux k (n-1))
...
--- ### Indefinite Loops
transUntil kloop (is, d, o) =
    if j /= 0 then
      (is, d, o)
    else
      transUntil kloop (js, dj, oj)
    where (j:js, dj, oj) = kloop (is, d, o)
